const String iconsPath = "assets/icons/";

class AppIcons {
  static const String placeHolder = 'assets/images/placeholder.png';
  static const String failbackImage200 = 'https://placehold.co/200';
  static const String failbackImage600 = 'https://placehold.co/600';
  static const String languageIcon = "${iconsPath}languageIcon.svg";
  static const String backArrowIcon = "${iconsPath}backArrowIcon.svg";
  static const String openEyeIcon = "${iconsPath}openEyeIcon.svg";
  static const String designIcon = "${iconsPath}design.svg";
  static const String favoriteIcon = "${iconsPath}favorite.svg";
  static const String galleryIcon = "${iconsPath}gallery.svg";
  static const String homeIcon = "${iconsPath}home.svg";
  static const String notificationIcon = "${iconsPath}notification_icon.svg";
  static const String projectsIcon = "${iconsPath}projects.svg";
  static const String structuralIcon = "${iconsPath}structural_icon.svg";
  static const String supplyIcon =
      "${iconsPath}goods_merchandise_stock_supply_vendibles_icon.svg";

  static const String paintRollIcon = "${iconsPath}paint_roll_icon.svg";
  static const String messageCommunicationIcon =
      "${iconsPath}message_communication_icon.svg";
  static const String greenArrowCircleRightIcon =
      "${iconsPath}green_arrow_circle_right.svg";

  static const String arrowCircleRightIcon =
      "${iconsPath}arrow_circle_right.svg";
  static const String colorWandSharptIcon =
      "${iconsPath}color_wand_sharp_icon.svg";
}
